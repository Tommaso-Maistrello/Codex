...
public class Campo {
	...
	public void casellaSpecifica() {
        ...
        while(!risposta.equals("no") && !risposta.equals("n")) {
	        ...
	        do {
	            System.out.print("Vuoi visualizzare una casella nello specifico (si/no)? ");
...
public void posizionaCartaIniziale(Giocatore giocatore) {
	...
	public void giocaCarta(Giocatore player) {
		...
		do {
			...
			if (esiste == true) {
				...
				if (esiste == false) {
					System.out.println(red+"La carta non si può giocare nelle coordinate fornite"+reset+"\n");// controlla se la coordintata è in un punto giocabile cioè se uno dei 4 angoli tocca un'altra carta
				}
			}
	...
	public boolean controlloRequisiti(Giocatore player, int id) {
        ...
        if(requisiti== false) {
             ...
             System.out.println("Al momento il numero di regni tra quelli richiesti è: numero animali = "+ ANIMALE +", numero insetti = "+INSETTI+", numero funghi = "+FUNGHI+", numero vegetale = "+VEGETALE);
